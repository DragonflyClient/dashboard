...
const types = {
  ...
  store: {
    ...
    bgColor: 'bg-drgn-plight',
    border: 'border-left-drgn-plight',
  },
};
...
function listNotifications() {
  ...
  notifications.reverse().forEach(notification => {
    ...
    const sendTime =
      new Date().getTime() < notification.sendTime
        ? moment(new Date().getTime()).fromNow()
        : moment(notification.sendTime).fromNow();
    var nth = 0;
    ...
    notificationList.innerHTML += `
    <div data-nid="${notification._id}" data-nread="${notification.read}" onclick='notificationAction("${
      notification.action && notification.action.type === 'open_url' ? notification.action.target : ''
    }", "${notification._id}", ${notification.read})' class="${
      !notification.read ? type.border : ''
    } notification-item dropdown-item d-flex align-items-center cursor-pointer">
        <div class="mr-3">
        ...
        <div>
            <div class="small text-gray-500">${sendTime}${
